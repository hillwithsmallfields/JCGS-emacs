;;;; autocue.el
;;; Time-stamp: <2007-01-29 19:07:24 jcgs>
...
(defgroup autocue nil
  "Controls for autocue, a function which steps through a buffer for the user
to read it without having to scroll manually.")

(defcustom autocue:start-hooks nil
  "Hooks to run on starting autocue."
  :group 'autocue
  :type 'hook)

(defcustom autocue:secs-per-step 1
  "How many seconds to pause per step."
  :group 'autocue
  :type 'integer)

(defcustom autocue:lines-per-step 1
...
(defcustom autocue:message-keep 1
  "How many more steps to leave a message up for."
  :group 'autocue
  :type 'integer)

(defcustom autocue:kill-buffer-at-end nil
  "Whether to kill the buffer at the end of reading it."
  :group 'autocue
  :type 'boolean)
