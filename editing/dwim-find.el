...
;;; Time-stamp: <2007-03-03 21:03:30 jcgs>
  ...
  (cond
   ((memq major-mode '(emacs-lisp-mode lisp-interaction-mode))
    (let ((variable (variable-at-point))
      ...
      ;; (message "Finding variable %S or function %S" variable function)
      (cond
       ((symbolp variable)
	;; (message "Finding variable %S" variable)
	(find-variable-other-window variable))
       ((symbolp function)
	;; (message "finding function %S" function)
	(cond
	 ((eq function 'require)
	  (find-library
	   (save-excursion
	 ...
	 (t
	  (find-function-other-window (function-called-at-point)))))
       ((library-which (symbol-name (thing-at-point 'symbol)))
	(find-library (symbol-name (thing-at-point 'symbol))))
       (t (message "Don't know how to find this lisp thing")))))
   ...
   (t
	...
	(find-tag (thing-at-point 'symbol))
      (error "Don't know how to find this")))))
