...
;;; Time-stamp: <2009-04-06 12:36:44 jcgs>
...
destroy the buffer."
  ...
  (save-window-excursion
    (dolist (file (directory-files tree t pattern))
      (message "%s?" file)
      (cond
...
(defun replace-string-in-tree (old new tree &optional matching regexp)
  "Replace all occurrences of OLD by NEW in files in directory TREE.
Optionally only in the files matching MATCHING.
Optionally make it a REGEXP."
...
sDo it in files matching: ")
  (let ((files (directory-files tree t nil t)))
	...
	(if (file-directory-p file)
		...
		(replace-string-in-tree old new file regexp))
	  (if (or (null matching)
		...
		(let ((done 0)
		  ...
		  (save-excursion
		    (goto-char (point-min))
		    (while (funcall search-function old nil t)
		      (setq done (1+ done))
