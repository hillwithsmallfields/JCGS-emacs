...
(setq try-fonts-old-font (cdr (assoc 'font (frame-parameters)))
      try-fonts-old-width (frame-width)
      try-fonts-old-height (frame-height))

(defun try-fonts-restore ()
  (interactive)
  (set-default-font try-fonts-old-font)
  (set-frame-width try-fonts-old-width)
  (set-frame-height try-fonts-old-height))
...
(defvar screen-setups
  '(("normal" "6x10" 80 24)
    ("very big" "-*-lucidatypewriter-*-*-*-*-*-240-*-*-*-*-*-*" 60 24)
...
(defun toggle-screen-setup ()
  "Switch between two common screen setups."
  (interactive)
  (cond
   ((equal current-screen-setup-name "normal")
    (use-screen-setup "largish lucida"))
   ((equal current-screen-setup-name "largish lucida")
    (use-screen-setup "normal"))))

(global-set-key [ Scroll_Lock ] 'toggle-screen-setup)
